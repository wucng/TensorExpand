...
# Functions and classes for loading and using the vgg16 model.
import vgg16 # 模型文件
import PIL.Image
import glob
...
def load_image(image_path):
    ...
    for path in img_path:
        image = np.array(PIL.Image.open(path).resize((224, 224))) #/255.
        imgs.append(image)
...
x = tf.placeholder(tf.float32, shape=[None, 7,7,512], name='x') # [n,2048]
...
pool2 = tf.layers.max_pooling2d(conv2, 2, 2, padding='SAME')  # [-1,2,2,256]

conv3 = tf.layers.conv2d(pool2, 2, 3, padding='SAME', activation=tf.nn.relu)
pool3 = tf.layers.max_pooling2d(conv3, 2, 2, padding='SAME')  # [-1,1,1,2]

# y_pred = tf.layers.dense(tf.reshape(pool3,[-1,512*2]), 2, activation=tf.nn.softmax) # [-1,2]
y_pred=tf.squeeze(pool3)
...
for epoch in range(20):
    ...
    for step in range(total_batchs):
        ...
        input_x=vgg16.feed_data(model,images=batch_x)
...
input_x=vgg16.feed_data(model,images=imgs_test[:20])
