...
'''
transfer_learning_ckpt.py
transfer_learning_pb.py
transfer_learning_pb2ckpt.py

（上面任意一个脚本）需执行完成一次，得到model.ckpt文件即可，才能执行本脚本
...
with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
        ...
        fc=tf.get_default_graph().get_tensor_by_name("D/coding_layer/Sigmoid:0")
        y_pred=tf.get_default_graph().get_tensor_by_name("D/output/Softmax:0")
