...
'''
# 网络微调只用到卷积层部分，理论上include_top=False 可行 (include_top=False 后面会报错 不知道为什么？)
model = VGG16(include_top=True, weights='imagenet') # 模型下载到 ~/.keras/models
...
# 在网络微调中，我们让预先训练的VGG16模型block5和block4层也进行参数更新（也可以尝试更新所有卷积层部分的网络），trainable=True。
