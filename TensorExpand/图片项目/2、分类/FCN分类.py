...
def convolutional_neural_network(X,keep_prob,training):
    ...
    conv3 = tf.add(conv3, pool1)
    conv3 = tf.layers.batch_normalization(conv3, training=training)
    decode=tf.layers.conv2d_transpose(conv3,10,15,1,padding='valid',activation=tf.nn.relu) # [-1,28,28,10]
    decode = tf.layers.batch_normalization(decode, training=training)

    decode=tf.reduce_max(decode,-1) # [-1,28,28,1]

    return tf.reshape(decode,[-1,28,28])


# 使用数据训练神经网络
def train_neural_network(X, Y,keep_prob,training,mnist):
    decode = convolutional_neural_network(X,keep_prob,training) # [-1,28,28]
    ...
    # 监督模型
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=decode, labels=tf.reshape(Y,[-1,28,28])))

    # cost = tf.losses.mean_squared_error(predictions=decode, labels=tf.reshape(Y, [-1, 28, 28]))
    optimizer = tf.train.AdamOptimizer().minimize(cost)  # learning rate 默认 0.001

    correct_prediction = tf.equal(decode, tf.reshape(Y,[-1,28,28]))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(total_batches):
                ...
                _, c = sess.run([optimizer, cost], feed_dict={X: x, Y: x,keep_prob:FLAGS.keep_hold,training:FLAGS.training})
                epoch_costs = np.append(epoch_costs, c)

            # print("Epoch: ", epoch,'|', " un_Loss: ", np.mean(un_epoch_costs))

            accuracy_in_train_set = sess.run(accuracy,
                                             feed_dict={X: mnist.train.images[:500], Y: mnist.train.images[:500],keep_prob:1.,training:True})
            accuracy_in_test_set = sess.run(accuracy, feed_dict={X: mnist.test.images, Y: mnist.test.images,keep_prob:1.,training:True})
            print("Epoch: ", epoch, '|', " Loss: ", np.mean(epoch_costs), '|', " Accuracy: ", accuracy_in_train_set,
                  '||',accuracy_in_test_set)

            save_path=saver.save(sess,os.path.join(FLAGS.checkpointDir,'model.ckpt'),global_step=epoch)
            print(save_path)
...
def main(_):
    ...
    X = tf.placeholder(tf.float32, [None, 28 * 28 * 1]) # [None,28*28*1]
    Y = tf.placeholder(tf.float32,[None,28*28*1]) # [-1,28*28*1] --->[-1,28,28]
