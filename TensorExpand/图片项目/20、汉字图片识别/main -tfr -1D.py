...
learning_rate=tf.train.polynomial_decay(0.01,global_step,10000,1e-5)
epochs=1
...
net=x # [n,32,32]
branch1 =tf.layers.conv1d(net,64,3,1,padding='valid',activation=tf.nn.relu) # [n,30,64]
branch1=slim.batch_norm(branch1,is_training=is_training)
branch2 =tf.layers.conv1d(net,64,5,1,padding='valid',activation=tf.nn.relu) # [n,28,64]
branch2=slim.batch_norm(branch2,is_training=is_training)
branch3 =tf.layers.conv1d(net,64,7,1,padding='valid',activation=tf.nn.relu) # [n,26,64]
branch3=slim.batch_norm(branch3,is_training=is_training)

net1=tf.concat([branch1, branch2, branch3],axis=1) #[n,84,64]

branch1 =tf.layers.conv1d(net,128,3,1,padding='valid',activation=tf.nn.relu) # [n,30,128]
branch1=slim.batch_norm(branch1,is_training=is_training)
branch2 =tf.layers.conv1d(net,128,5,1,padding='valid',activation=tf.nn.relu) # [n,28,128]
branch2=slim.batch_norm(branch2,is_training=is_training)
branch3 =tf.layers.conv1d(net,128,7,1,padding='valid',activation=tf.nn.relu) # [n,26,128]
branch3=slim.batch_norm(branch3,is_training=is_training)

net2=tf.concat([branch1, branch2, branch3],axis=1) #[n,84,128]

branch1 =tf.layers.conv1d(net,256,3,1,padding='valid',activation=tf.nn.relu) # [n,30,256]
branch1=slim.batch_norm(branch1,is_training=is_training)
branch2 =tf.layers.conv1d(net,256,5,1,padding='valid',activation=tf.nn.relu) # [n,28,256]
branch2=slim.batch_norm(branch2,is_training=is_training)
branch3 =tf.layers.conv1d(net,256,7,1,padding='valid',activation=tf.nn.relu) # [n,26,256]
branch3=slim.batch_norm(branch3,is_training=is_training)

net3=tf.concat([branch1, branch2, branch3],axis=1) #[n,84,256]

network=tf.concat([net1, net2, net3],axis=-1) # [n,84,448]

cell = tf.contrib.rnn.BasicLSTMCell(448)
cell=tf.contrib.rnn.MultiRNNCell([cell]*3)
cell=tf.contrib.rnn.DropoutWrapper(cell,keep_rate)
init_state = cell.zero_state(batch_size, dtype=tf.float32)
outputs, final_state = tf.nn.dynamic_rnn(cell, network, initial_state=init_state, time_major=False)

fc1=tf.layers.dense(outputs[:,-1,:],2048,activation=tf.nn.relu)
fc1=tf.nn.dropout(fc1,keep_rate)
prediction=tf.layers.dense(fc1,num_class,activation=None) # [none,3755]

'''
net = tf.expand_dims(net, 2)  # [n,84,1,32]

net = tf.reduce_max(net, [1, 2]) # [none,32]

net=tf.nn.dropout(net,keep_rate)

prediction=tf.layers.dense(net,num_class,activation=tf.nn.softmax,) # [none,3755]
'''
...
if __name__=="__main__":
    ...
    if train==1:
        x_train_batch, y_train_batch = tfrecord_to_numpy(glob('./data/train-*.tfrecords'))
        acc_last=0.0
        steps = 895035 // batch_size
    if train==0:
        x_test_batch, y_test_batch = tfrecord_to_numpy(glob('./data/test-*.tfrecords'))
        acc_list = []
