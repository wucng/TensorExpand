...
#获得buckets路径
parser.add_argument('--buckets', type=str, default='./Data_Collection',
...
data_path=glob.glob(os.path.join(FLAGS.buckets,'*.jpg'))
imgs=[]
for path in data_path:
    img=Image.open(path).convert('L')
    img=img.resize((28,28))
    imgs.append(np.array(img,np.uint8)/255.)

imgs=np.array(imgs,np.float32) # [-1,28,28,1]
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(10):
            ...
            for step in range(total_batches):
                ...
                x = imgs[step * batch_size:(step + 1) * batch_size][:,:,:,np.newaxis]
