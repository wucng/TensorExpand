...
def loss(logits, labels, num_classes, head=None):
    ...
    with tf.name_scope('loss'):
        logits = tf.reshape(logits, (-1, num_classes)) # [batch_size*width*height, num_classes]
        epsilon = tf.constant(value=1e-4)
        labels = tf.to_float(tf.reshape(labels, (-1, num_classes))) # [batch_size*width*height, num_classes]
