...
NUM_OF_CLASSESS = 21 # 151 # 类别数 算上背景4个类别 1+3（背景默认为0 必须算上）
IMAGE_SIZE = 224 # 图像大小

train_images_num=200
val_images_num=50
...
def inference(image, keep_prob):
    ...
    # model_data=None
    # processed_image=image/255. # 不使用VGG预训练的模型参数

    with tf.variable_scope("inference"):
        image_net = vgg_net(weights, processed_image)
        # image_net = vgg_net(processed_image)
        conv_final_layer = image_net["conv5_3"] # [n,14,14,512]

        pool5 = utils.max_pool_2x2(conv_final_layer) # [n,7,7,512]

        W6 = utils.weight_variable([7, 7, 512, 4096], name="W6")
        b6 = utils.bias_variable([4096], name="b6")
        conv6 = utils.conv2d_basic(pool5, W6, b6) # [n,7,7,4096]
        ...
        conv7 = utils.conv2d_basic(relu_dropout6, W7, b7) # [n,7,7,4096]
        ...
        conv8 = utils.conv2d_basic(relu_dropout7, W8, b8) # [n,7,7,151 ]  NUM_OF_CLASSESS = 151
        ...
        conv_t1 = utils.conv2d_transpose_strided(conv8, W_t1, b_t1, output_shape=tf.shape(image_net["pool4"])) # [n,14,14,512]
        fuse_1 = tf.add(conv_t1, image_net["pool4"], name="fuse_1") # [n,14,14,512]
    ...
    return tf.expand_dims(annotation_pred, dim=3), conv_t3 # [n,224,224,1] , [n,224,224,151]
...
def main(argv=None):
    ...
    pred_annotation, logits = inference(image, keep_probability) # [n,224,224,1] , [n,224,224,151]
    ...
    # loss = tf.reduce_mean((tf.nn.sparse_softmax_cross_entropy_with_logits(logits=tf.reshape(logits,[-1,NUM_OF_CLASSESS]),
    ...
    # f 为JPG路径   annotation_file 为对应的mask路径
    dataset_train=Pascal_VOC_2007_data.ShapesDataset()
    dataset_train.load_shapes(train_images_num)
    train_records = dataset_train.image_info

    dataset_val = Pascal_VOC_2007_data.ShapesDataset()
    dataset_val.load_shapes(val_images_num)
