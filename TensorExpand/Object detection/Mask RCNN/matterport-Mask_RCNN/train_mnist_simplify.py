...
class ShapesDataset(utils.Dataset):
    def load_shapes(self, count, height, width, train=True):
        ...
        for i in range(count):
            comb_image, mask, class_ids = self.random_comb(mnist, height, width, train)
    ...
    def random_comb(self, mnist, height, width, train=True):
        if train:
            train = mnist.train
        else:
            train = mnist.test
        num_images = train.num_examples
        indexs = random.choices(np.arange(0, num_images), k=16)  # 随机选择16个索引值
        indexs = np.asarray(indexs, np.uint8).reshape([4, 4])
        class_ids = train.labels[indexs.flatten()] + 1  # 0~9数字对应标签1~10,0默认为背景
        ...
        for i in range(4):
            for j in range(4):
                ...
                img_data = train.images[indexs[i, j]]
...
dataset_val.load_shapes(100, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1], False)
dataset_val.prepare()
