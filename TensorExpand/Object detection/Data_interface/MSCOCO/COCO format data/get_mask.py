...
def mask2polygons(mask):
    ...
    for cont in contours[1]:
        [polygons.append(i) for i in list(cont.flatten())]
        # map(polygons.append,list(cont.flatten()))
    return polygons # list(contours[1][0].flatten()) [x1,y1,x2,y2,...]
...
if __name__=='__main__':
    ...
    mask = polygons_to_mask2([height, width], polygons)
    ...
    seg = mask2polygons(mask)
    polygons = []
    for i in range(0, len(seg), 2):
        polygons.append([seg[i], seg[i + 1]])

    mask2=polygons_to_mask2([height, width], polygons)
    plt.subplot(132)
    ...
    # 使用opencv在mask图上画出
    cv2.rectangle(mask, tuple(bbox[:2]), tuple(bbox[2:]), 255)
