...
parser.add_argument("-e", "--epochs", help="The number of iterations", type=int, default=1) # 迭代次数
parser.add_argument("-s", "--steps", help="The number of steps", type=int, default=10) # 每个迭代执行的步数
parser.add_argument("-b", "--batch_size", help="number of records per batch", type=int, default=3) # 每步训练的样本数
...
class Data_info(object):
    ...
    def load_image(self,img_paths):
        ...
        for path in img_paths:
            ...
            img.append(cv2.resize(cv2.imread(path),(500,400))) # 转成400x500

        img=np.asarray(img,np.float16) # [batch_size,h,w,c]

        # 数据归一化
        self.imgs=(img-np.min(img,0,keepdims=True))/(np.max(img,0,keepdims=True)-np.min(img,0,keepdims=True))
