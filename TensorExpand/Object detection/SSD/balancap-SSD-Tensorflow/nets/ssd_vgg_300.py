...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_300_vgg', [inputs], reuse=reuse):
        ...
        net = slim.repeat(inputs, 2, slim.conv2d, 64, [3, 3], scope='conv1') # [1,300,300,64]
        end_points['block1'] = net
        net = slim.max_pool2d(net, [2, 2], scope='pool1') # [1,150,150,64]
        # Block 2.
        net = slim.repeat(net, 2, slim.conv2d, 128, [3, 3], scope='conv2') # [1,150,150,128]
        end_points['block2'] = net
        net = slim.max_pool2d(net, [2, 2], scope='pool2') # [1,75,75,128]
        # Block 3.
        net = slim.repeat(net, 3, slim.conv2d, 256, [3, 3], scope='conv3')
        end_points['block3'] = net
        net = slim.max_pool2d(net, [2, 2], scope='pool3') # [n,38,38,256]
        # Block 4.
        net = slim.repeat(net, 3, slim.conv2d, 512, [3, 3], scope='conv4')
        end_points['block4'] = net
        net = slim.max_pool2d(net, [2, 2], scope='pool4') # [n,19,19,512]
        # Block 5.
        net = slim.repeat(net, 3, slim.conv2d, 512, [3, 3], scope='conv5')
        end_points['block5'] = net
        net = slim.max_pool2d(net, [3, 3], stride=1, scope='pool5') # [n,19,19,512]

        # Additional SSD blocks.
        # Block 6: let's dilate the hell out of it!
        net = slim.conv2d(net, 1024, [3, 3], rate=6, scope='conv6') # [n,19,19,1024]
        end_points['block6'] = net
        net = tf.layers.dropout(net, rate=dropout_keep_prob, training=is_training)
        # Block 7: 1x1 conv. Because the fuck.
        net = slim.conv2d(net, 1024, [1, 1], scope='conv7') # [n,19,19,1024]
        ...
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 256, [1, 1], scope='conv1x1') # [n,19,19,256]
            net = custom_layers.pad2d(net, pad=(1, 1)) # [n,21,21,256]
            net = slim.conv2d(net, 512, [3, 3], stride=2, scope='conv3x3', padding='VALID') # [n,10,10,512]
        end_points[end_point] = net
        end_point = 'block9'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 128, [1, 1], scope='conv1x1') # [n,10,10,128]
            net = custom_layers.pad2d(net, pad=(1, 1)) # [n,12,12,128]
            net = slim.conv2d(net, 256, [3, 3], stride=2, scope='conv3x3', padding='VALID') # [n,5,5,256]
        end_points[end_point] = net
        end_point = 'block10'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 128, [1, 1], scope='conv1x1') # [n,5,5,128]
            net = slim.conv2d(net, 256, [3, 3], scope='conv3x3', padding='VALID') # [n,3,3,256]
        end_points[end_point] = net
        end_point = 'block11'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 128, [1, 1], scope='conv1x1') # [n,3,3,128]
            net = slim.conv2d(net, 256, [3, 3], scope='conv3x3', padding='VALID') # [n,1,1,256]
        end_points[end_point] = net
