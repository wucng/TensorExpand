...
X_ = tf.placeholder('float', [None, 28 * 28*1])
# Y = tf.placeholder('float')


# 使用数据训练神经网络
def train_neural_network(X,X_):
    decode = convolutional_neural_network(X,False) # 自编码器 （encode+decode）
    decode_ = convolutional_neural_network(X_,True) # 生成器 （decode）
    # 非监督模型1
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for step in range(total_batches):
                ...
                x_=np.random.random([batch_size,28*28*1])
        ...
        # 生成mnist
        decode_images=sess.run(decode_,feed_dict={X_:np.random.random([1,28*28*1])})
