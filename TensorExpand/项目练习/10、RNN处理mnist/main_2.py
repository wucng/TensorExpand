...
def recurrent_neural_network(data):
    ...
    return ouput,status,initial_state,lstm_cell
...
def train_neural_network(X, Y):
    predict,status,initial_state,_ = recurrent_neural_network(X)
...
def test_neural_network(X, Y):
    predict,status,initial_state,lstm_cell = recurrent_neural_network(X)
    ...
    with tf.Session() as session:
        ...
        [acc,state_]=session.run([accuracy, status], feed_dict={X: mnist.test.images[:64].reshape(-1, chunk_n, chunk_size),Y: mnist.test.labels[:64], initial_state: state_})
