...
def recurrent_neural_network(data):
    ...
    initial_state=lstm_cell.zero_state(batch_size,tf.float32) # 限制了每次输入的数据只能是batch_size个数据
    '''
    ...
    '''
    # dynamic_rnn
    outputs, status = tf.nn.dynamic_rnn(lstm_cell, data, initial_state=initial_state, dtype=tf.float32)
    ouput = tf.add(tf.matmul(outputs[:,-1,:], layer['w_']), layer['b_'])


    return ouput,status,initial_state
...
def train_neural_network(X, Y):
    predict,status,initial_state = recurrent_neural_network(X)
    ...
    with tf.Session() as session:
        ...
        for epoch in range(epochs):
            for i in range(int(mnist.train.num_examples // batch_size)):
                ...
                _, c,state = session.run([optimizer, cost_func,status], feed_dict=feed_dict)
                epoch_loss += c
            print(epoch, ' : ', epoch_loss)

        correct = tf.equal(tf.argmax(predict, 1), tf.argmax(Y, 1))
        accuracy = tf.reduce_mean(tf.cast(correct, 'float'))
        print('准确率: ', accuracy.eval({X: mnist.test.images[:batch_size].reshape(-1, chunk_n, chunk_size), Y: mnist.test.labels[:batch_size],initial_state:state}))
