...
验证码图像对应4个字符，放在一行（每一行对应一个样本）对应（4列）4个标签
...
def get_next_batch(batch_size=128):
    ...
    batch_y = np.zeros([batch_size, MAX_CAPTCHA * CHAR_SET_LEN]) # [batch ,4*63]
    ...
    for i in range(batch_size):
        ...
        batch_y[i, :] = text2vec(text) # [4*63,]

    return batch_x, batch_y # [batch,60*160],[batch,4*63]

####################################################################

X = tf.placeholder(tf.float32, [None, IMAGE_HEIGHT * IMAGE_WIDTH]) # [batch,60*160]
Y = tf.placeholder(tf.float32, [None, MAX_CAPTCHA * CHAR_SET_LEN]) # [batch ,4*63]
