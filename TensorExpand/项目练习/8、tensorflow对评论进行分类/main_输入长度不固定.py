...
def rnn_net_1(x):
    ...
    inputs = slim.embed_sequence(tf.cast(x, tf.int32), vocab_size=len(lex), embed_dim=n_layer_1) # # [batch_size,-1,n_layer_1]
...
# 输入不定长
X = tf.placeholder('float', [batch_size, None]) # 不定长
Y = tf.placeholder('float',[batch_size,None]) # 定长

# 使用数据训练神经网络
def train_neural_network(X, Y):
    # predict = neural_network(X)
    # predict = rnn_net(X)
    predict = rnn_net_1(X) # [-1,2]
