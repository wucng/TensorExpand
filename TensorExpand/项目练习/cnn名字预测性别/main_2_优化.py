...
'''
RNN 参考：http://lib.csdn.net/article/aiframework/61965
输入：[-1,8]
标签：[-1,2]
...
作者：AfterlifeYeah
链接：https://www.jianshu.com/p/3dbeb3ab9aa3
來源：简书
著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。

'''
import tensorflow as tf
from tensorflow.contrib import slim # cnn
...
def neural_network2(vocabulary_size, embedding_size=128, num_filters=128):
    '''
    使用CNN， 3种核 的卷积 再按最后维度拼接，最后是输出层
...
def neural_network(vocabulary_size, embedding_size=128, num_filters=128):
    ...
    with tf.device('/cpu:0'), tf.name_scope("embedding"):
        ...
        embedded_chars = slim.embed_sequence(X, vocabulary_size, embedding_size) # X 必须是二维的 [-1,8,128]
        # embedded_chars_expanded = tf.expand_dims(embedded_chars, -1) # [-1,8,128,1] 变成4维方便使用卷积
        ...
        outputs, final_state = tf.nn.static_rnn(cell, inputs, initial_state=init_state)
