...
def dnn_net(x,dropout,n_classes): # [none,19*19*4]
    # x = tf.reshape(x, shape=[batch_size, img_h, img_w, img_c])
    fc1=tf.layers.dense(x,256,activation=tf.nn.relu) # [none,256]
    # fc1=tf.layers.dropout(fc1,dropout) # 这种使用占位符会报错
    fc1=tf.nn.dropout(fc1,dropout)
    out=tf.layers.dense(fc1,n_classes) # [none,2]

    return out

# Construct model
pred = dnn_net(x, keep_prob,n_classes)
