...
xx=data[:,:-1].astype(np.float32) # [-1,19*19*4]
...
class DNN(nn.Module):
    def __init__(self):
        ...
        self.fc1=nn.Sequential(
            nn.Linear(img_h*img_w*img_c,256),
            nn.ReLU(),
            nn.Dropout(dropout)
        )

        self.out = nn.Linear(256, n_classes)   # fully connected layer, output 10 classes
...
# cnn = CNN().double()
dnn=DNN()
print(dnn)  # net architecture

optimizer = torch.optim.Adam(dnn.parameters(), lr=learning_rate)   # optimize all cnn parameters
