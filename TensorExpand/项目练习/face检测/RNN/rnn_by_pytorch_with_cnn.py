...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # input shape (-1,4, 19, 19)
            nn.Conv2d(img_c, 32, 5, 1, 2),  # output shape (-1,32, 19, 19)
            nn.ReLU(),  # activation
            nn.MaxPool2d(2),  # output shape (-1,32, 9, 9)
        )
    ...
    def forward(self, x):
        # x shape (batch, time_step, input_size) [-1,4,19,19]
        # r_out shape (batch, time_step, output_size) [-1,32,256]
        # h_n shape (n_layers, batch, hidden_size)
        # h_c shape (n_layers, batch, hidden_size)
        # x=self.input(x)
        x=self.conv1(x) # [-1,32,9,9]
        x = x.view(-1,32,hide_layers) #  [-1,32,9*9]
        r_out, (h_n, h_c) = self.rnn(x, None)  # None represents zero initial hidden state [-1,32,256]
        # choose r_out at the last time step
        out = self.out(r_out[:, -1, :]) # [-1,2]  ; r_out[:, -1, :]  shape [-1,256]
        return out # [-1,2]
