...
NPZ_FILE = 'wine.data.npz' #"HIGGS.csv.gz.npz"  # numpy compressed file containing "data" array
...
def train_boosted_trees(flags_obj):
  ...
  train_input_fn, feature_names, feature_columns = make_inputs_from_np_arrays(
      features_np=train_data[:, 1:], label_np=np.asarray(train_data[:, 0:1],np.float32)-1) # 标签必须从0 开始
  eval_input_fn = make_eval_inputs_from_np_arrays(
      features_np=eval_data[:, 1:], label_np=np.asarray(eval_data[:, 0:1],np.float32)-1) # 标签必须从0 开始
...
def define_train_higgs_flags():
  ...
  flags.DEFINE_integer(
      name="train_count", default=100,
      help=help_wrap("Number of train examples within the data."))
  flags.DEFINE_integer(
      name="eval_start", default=100,
      help=help_wrap("Start index of eval examples within the data."))
  flags.DEFINE_integer(
      name="eval_count", default=78,
      help=help_wrap("Number of eval examples within the data."))

  flags.DEFINE_integer(
      "n_trees", default=10, help=help_wrap("Number of trees to build."))
  flags.DEFINE_integer(
      "max_depth", default=3, help=help_wrap("Maximum depths of each tree."))
  flags.DEFINE_float(
