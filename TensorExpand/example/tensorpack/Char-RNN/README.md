[字符级RNN语言模型](https://github.com/tensorpack/tensorpack/tree/master/examples/Char-RNN)

要训练，在`input.txt`中创建一些语料库，然后运行`./char-rnn.py train`。 一些语料库可以玩：http://cs.stanford.edu/people/karpathy/char-rnn/

要生成文本，请参阅`./char-rnn.py sample --help`以获取选项

生成linux内核源代码：(将下面的代码写入`input.txt`作为语料库，作为训练的样本)
...
```
生成paper:(在我的个人文件夹中训练深度学习论文，通过pdftotext转换为凌乱的纯文本格式）。
...
```
有关此主题的更多有趣故事，请参阅[Andrej Karpathy的博客](http://karpathy.github.io/2015/05/21/rnn-effectiveness/)。