...
weights = {
    ...
    'in':tf.Variable(tf.random_normal([3, 3, 1, 16])),
    'out': tf.Variable(tf.random_normal([2*n_hidden, n_classes]))
}
biases = {
    'in':tf.Variable(tf.random_normal([16])),
...
def BiRNN(x, weights, biases):
    ...
    x = tf.nn.bias_add(tf.nn.conv2d(x, weights['in'], strides=[1, 1, 1, 1], padding='SAME'), biases['in'])
