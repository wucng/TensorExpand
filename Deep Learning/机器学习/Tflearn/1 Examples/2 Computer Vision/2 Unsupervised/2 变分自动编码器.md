经过训练的变分自动编码器（VAE）可生成数字图像。

<font color=#FF00FF size=5>使用变分自动编码器从噪音中产生数字图像。</font>
MNIST手写数字被用作训练示例。

原理特别简单，只需要在编码过程给它增加一些限制，迫使其生成的隐含向量能够粗略的遵循一个标准正态分布，这就是其与一般的自动编码器最大的不同

[花式解释AutoEncoder与VAE](https://www.jianshu.com/p/78f91e8aafed)
