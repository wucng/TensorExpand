...
必须安装 GPU 版的 Torch, [点击这里查看如何安装](https://morvanzhou.github.io/tutorials/machine-learning/torch/1-2-install/)

# 用 GPU 训练 CNN
这份 GPU 的代码是依据之前[这份CNN的代码](https://github.com/MorvanZhou/PyTorch-Tutorial/blob/master/tutorial-contents/401_CNN.py)修改的. 大概修改的地方包括将数据的形式变成 GPU 能读的形式, 然后将 CNN 也变成 GPU 能读的形式. 做法就是在后面加上 .cuda(), 很简单.
    ...
    for step, (x, y) in enumerate(train_loader):   # gives batch data, normalize x when iterate train_loader
        ...
        if step % 50 == 0:
            ...
            if HAS_SK:
                ...
                plot_only = 500
                low_dim_embs = tsne.fit_transform(last_layer.cpu().data.numpy()[:plot_only, :]) # 从GPU转到cpu() 才能画图
                labels = test_y.cpu().numpy()[:plot_only] # 从GPU转到cpu() 才能画图
